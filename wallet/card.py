...
class Card:
    ...
    def display_card(self):
        print(f"La carte {self.name}, ayant le numéro {self.number} a actuellement en solde: {self.balance} ariary.")

def handle_cards():
    card_menu = "    Choisissez une action à faire: \n      1. Ajouter une carte\n      2. Retirer une carte\n      3. Voir les cartes\n      4. Recharger une carte\n      5. Débiter une carte\n Votre choix: "
...
def add_card():
    ...
    balance = int(input("    Veuillez entrer le montant du solde de votre carte: "))
    if(balance < 0):
...
def remove_card():
    ...
    if not exists:
        ...
        cards = [card for card in cards if card.number.lower() != to_remove.lower()]
...
def refill_card():
    ...
    if not exists:
        ...
        if(amount < 0):
            ...
            cards = [Card(card.number, card.name, card.balance + amount) if card.number.lower() == number.lower() else card for card in cards]
            print("\n    Balance de la carte mise à jour avec succès.\n")
            make_transactions(f"recharge sur la carte {number}", amount=amount)
            from menu import show_menu
            show_menu()
...
def debit_card():
    ...
    if not exists:
        ...
        if(amount < 0):
            ...
            make_transactions(f"retrait sur la carte {number}", amount=amount)
            from menu import show_menu
            show_menu()
